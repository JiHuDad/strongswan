    ...
    private_extsock_json_parser_t *this, cJSON *auth_json, bool is_local)
{
    ...
    if (j_auth_type && cJSON_IsString(j_auth_type) && j_auth_type->valuestring) {
        ...
        if (streq(auth_type_str, "psk")) {
            ...
            // MULTIPLE CA 지원: CA 인증서들 로딩 (Root CA, Intermediate CA 등)
            linked_list_t *ca_certs_list = linked_list_create();
            int ca_count = 0;
            
            // Single CA cert (backward compatibility)
                ...
                cJSON_ArrayForEach(ca_cert_item, j_ca_certs) {
                    if (cJSON_IsString(ca_cert_item) && ca_cert_item->valuestring) {
                        ...
                        if (ca) {
                            ...
                            ca_count++;
                            EXTSOCK_DBG(2, "CA certificate #%d loaded from: %s", ca_count, ca_cert_item->valuestring);
                        } else {
            ...
            if (j_ca_chain && cJSON_IsObject(j_ca_chain)) {
                ...
                if (j_root_ca && cJSON_IsString(j_root_ca) && j_root_ca->valuestring) {
                    ...
                    if (root_ca) {
                        ...
                        ca_count++;
                        EXTSOCK_DBG(1, "Root CA loaded from: %s", j_root_ca->valuestring);
                    }
                }
                ...
                if (j_intermediate_cas && cJSON_IsArray(j_intermediate_cas)) {
                    cJSON *intermediate_item;
                    int intermediate_count = 0;
                    cJSON_ArrayForEach(intermediate_item, j_intermediate_cas) {
                        if (cJSON_IsString(intermediate_item) && intermediate_item->valuestring) {
                            certificate_t *intermediate_ca = this->cert_loader->load_certificate(this->cert_loader, intermediate_item->valuestring);
                            if (intermediate_ca) {
                                ca_certs_list->insert_last(ca_certs_list, intermediate_ca);
                                ca_count++;
                                intermediate_count++;
                                EXTSOCK_DBG(1, "Intermediate CA #%d loaded from: %s", intermediate_count, intermediate_item->valuestring);
                            }
                        }
                    }
                    EXTSOCK_DBG(1, "Total intermediate CAs loaded: %d", intermediate_count);
                }
            }
            
            EXTSOCK_DBG(1, "Total CA certificates loaded: %d", ca_count);
            
            // Phase 3: Advanced trust chain validation (if CAs and subject cert available)
            if (ca_count > 0 && cert) {
            ...
            if (ca_count > 0) {
                ...
                while (ca_enum->enumerate(ca_enum, &ca_cert_item)) {
                    ...
                    EXTSOCK_DBG(3, "CA certificate added to credential store and auth config");
                }
